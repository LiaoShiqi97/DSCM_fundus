...
from torch.distributions import Independent, Gumbel
...
from pyro.distributions import LowRankMultivariateNormal, MultivariateNormal, Normal, TransformedDistribution, Gumbel
...
class CustomELBO(TraceGraph_ELBO):
    ...
    def _get_trace(self, model, guide, args, kwargs):
        ...
        # weight the KL divergence with 0.1
        # model_trace.nodes['z']['scale'] = model_trace.nodes['z']['scale'] * 0.1
        # model_trace.nodes['z']['log_prob'] = model_trace.nodes['z']['log_prob'] * 0.1
        # model_trace.nodes['z']['log_prob_sum'] = model_trace.nodes['z']['log_prob_sum'] * 0.1
        # guide_trace.nodes['z']['scale'] = guide_trace.nodes['z']['scale'] * 0.1
        # guide_trace.nodes['z']['log_prob'] = guide_trace.nodes['z']['log_prob'] * 0.1
        # guide_trace.nodes['z']['log_prob_sum'] = guide_trace.nodes['z']['log_prob_sum'] * 0.1
...
class BaseVISEM(BaseSEM):
    ...
    def __init__(self, latent_dim: int, logstd_init: float = -5, enc_filters: str = '16,32,64,128', dec_filters: str = '128,64,32,16',
        ...
        self.img_shape = (3, 512 // self.downsample, 512 // self.downsample) if self.downsample > 0 else (3, 512, 512)
        ...
        self.context_dim = 3   #hidden = torch.cat([hidden, age_, sex, T2D], 1)
        ...
        if self.decoder_type == 'fixed_var':
            self.decoder = Conv2dIndepNormal(decoder, 16, 3)
        ...
        self.sex_logits = torch.nn.Parameter(torch.zeros([1, ], requires_grad=True))

        self.register_buffer('gumbel_loc', torch.zeros([2, ], requires_grad=False))
        self.register_buffer('gumbel_scale', torch.ones([2, ], requires_grad=False))
        ...
        pyro.module("age_flow_components", self.age_flow_components, update_module_params=True)
    ...
    @pyro_method
    def svi_guide(self, x, age, sex, T2D):
        self.guide(x, age, sex, T2D)

    @pyro_method
    def svi_model(self, x, age, sex, T2D):
        with pyro.plate('observations', x.shape[0]):
            pyro.condition(self.model, data={'x': x, 'sex': sex, 'age': age, 'T2D': T2D})()
    ...
    @pyro_method
    def reconstruct(self, x, age, sex, T2D, num_particles: int = 1):
        obs = {'x': x, 'sex': sex, 'age': age, 'T2D': T2D}
        ...
        for _ in range(num_particles):
            ...
            recon, *_ = pyro.poutine.condition(
                self.sample, data={'sex': sex, 'age': age, 'T2D': T2D, 'z': z})(x.shape[0])
            recons += [recon]
    ...
    def counterfactual(self, obs: Mapping, condition: Mapping = None, num_particles: int = 1):
        ...
        for _ in range(num_particles):
            ...
            #abduction
            age_ = self.age_flow_constraint_transforms.inv(obs['age'])
            T2D_context = torch.cat([obs['sex'], age_], dim=-1)
            T2D_logits = self.T2D_net(T2D_context.unsqueeze(0)).squeeze(0)
            Gumbel_dist = Gumbel(loc=torch.tensor([0.]).cuda(), scale=torch.tensor([1.]).cuda())

            def truncated_gumbel(alpha, truncation):
                gumbel = Gumbel_dist.sample() + alpha
                return -torch.log(torch.exp(-gumbel) + torch.exp(-truncation)) -alpha

            def topdown(alphas, k):
                topgumbel = Gumbel_dist.sample() + torch.log(sum( torch.exp(alphas))) - alphas[k]
                gumbels = []
            ...
            ks = obs['T2D'].int()
            exogeneous['T2D_base'] =torch.stack([torch.stack(topdown(T2D_logit, k)) for T2D_logit, k in zip(T2D_logits, ks)]).squeeze(-1).cuda()

            counter = pyro.poutine.do(pyro.poutine.condition(self.sample_scm, data=exogeneous), data=condition)(obs['x'].shape[0])

            counterfactuals += [counter]

        # note the order of ('x', 'z', 'age', 'sex', 'T2D') to match def model in conditional_sem.py
        counterfactual = {k: v for k, v in zip(('x', 'z', 'age', 'sex', 'T2D'), (torch.stack(c).float().mean(0) for c in zip(*counterfactuals)))}
        counterfactual['age']=counterfactual['age'].round()
        counterfactual['sex'] = counterfactual['sex'].round()
        counterfactual['T2D'] = counterfactual['T2D'].round()
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--latent_dim', default=1024, type=int, help="latent dimension of model (default: %(default)s)")
...
class SVIExperiment(BaseCovariateExperiment):
    ...
    def get_trace_metrics(self, batch):
        ...
        metrics['log p(T2D)'] = model.nodes['T2D']['log_prob'].mean()
